...
> 파일 경로: `C:\Users\crytonite7777\chall1.exe`{: .filepath}  
{: .prompt-tip}
...
```console
C:\Users\crytonite7777>chall1.exe
input :
```
...
조건을 파악하기 위해 디컴파일과 어셈블리 분석을 진행했습니다. <br> 먼저, IDA64로 디컴파일한 main 함수를 살펴보겠습니다.
...
> v4는 256바이트 크기의 입력값을 저장하는 배열입니다. <br>
`sub_140001000(v4)`{: .highlight} 함수가 입력값을 조건에 따라 판단하며, 반환값(True/False)에 <br>
따라 `"Correct"` 또는 `"Wrong"`을 출력합니다.
{: .prompt-tip}

이를 더 자세히 확인하기 위해 x64dbg로 어셈블리 분석을 진행했습니다.

## `Main` assembly
![](assets/img/Rev-basic-1/Main_asm.png)
_x64 Main_

> `call chall1.7FF634871000`{: .highlight} 호출 후 조건분기(je)를 통해 `"Correct"` 또는 `"Wrong"`이 출력됩니다. 여기서 `sub_140001000`{: .highlight}과 `chall1.7FF634871000`{: .highlight}은 동일한 함수입니다.
{: .prompt-tip}

그렇다면 이제 `sub_140001000`{: .highlight} 함수 내부를 분석해보도록 하겠습니다.

## `Sub` assembly
![](assets/img/Rev-basic-1/sub_function.png)
_x64 Sub_
